...
MODEL_CONFIDENCE_THRESHOLD = float(
    os.environ.get("AI_MODEL_CONFIDENCE", "0.25")
)
...
def fix_wrong_training_labels(category: str) -> str:

    # ===== FIX DATASET LABEL CONFUSION =====

    if category == "DAMAGED_SIGN":
        return "FLOODING"
...
def map_label_to_category(label: str) -> str:
    ...
    for key, category in CATEGORY_MAPPING.items():
        ...
        if key == normalized or key in normalized:
            ...
            return fix_wrong_training_labels(category)

    # If nothing matches, return original default
    return "DAMAGED_SIGN"
